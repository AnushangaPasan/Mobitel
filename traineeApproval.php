...
$sql1 = " UPDATE `tbl_rfid` SET `rfid_user_ID`='$T_id_app',`flag`='ACTIVE'WHERE rfid_Code = '$division' ";
...
// $sql1 = " UPDATE `tbl_rfid` SET `rfid_user_ID`='$T_id_app',`flag`='ACTIVE'WHERE rfif_Code = '55' ";
...
<th style="padding-top: 20px;"><label>RFID Number</label>
              <td style="width: 200px; padding-right: 30px;padding-top: 20px;padding-left: 10px;"><select class="form-control" name="ID_number" id="ID_number">
                ...
                <?php
                $sql = "SELECT *  FROM tbl_rfid WHERE flag = 'INACTIVE' ";
                $division_data = $con->query($sql);
                if ($division_data->num_rows >= 1) :

                  while ($row = mysqli_fetch_assoc($division_data)) :?>
                  <option value="<?php echo $row["rfid_Code"]?>"><?php echo $row["rfid_Code"]?></option>
                  <?php
                endwhile;
              endif;
...
<!--
<th style="padding-top: 20px;"><label>RFID Number</label>
              <td style="width: 200px; padding-right: 30px;padding-top: 20px;padding-left: 10px;"><select class="form-control" name="rfid_Code" id="rfid_Code">
                <option>--SELECT--</option>
                <?php
                $sql = "SELECT *  FROM tbl_rfid WHERE flag = 'INACTIVE' ";
                $RFID_data = $con->query($sql);
                if ($RFID_data->num_rows >= 1) :

                  while ($row = mysqli_fetch_assoc($RFID_data)) :?>
                  <option value="<?php echo $row["rfid_Code"]?>"><?php echo $row["rfid_Code"]?></option>
                  <?php
                endwhile;
              endif;
